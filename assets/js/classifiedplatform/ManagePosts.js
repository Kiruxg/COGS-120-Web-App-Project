...
ref.on('value', function(data){
   ...
   var refArray = []
    ...
    var postArray = [];
        ...
        if (userData !== undefined){ //the user has atleast 1 post
...
editPost = index => {
    var self = this;
    if(this.state.postArray[index].category === 'Events' ||
        this.state.postArray[index].category === 'Resources' 
      ){
...
ref.once('value', function(data){
    ...
    firebase.database().ref('users/' + currentUser.uid + '/Posts/'+ keys[index]).remove()
})
//this.state.refArray[index].remove()
    ...
    managePost = () => {
         ...
         if(this.state.postArray !== undefined){
            ...
            return(
				<div className='noresult'>You do not have any active posts.</div>
    ...
    return this.state.postArray.map((item, i) => {
        ...
        <section className='listloop viewPost box' key={i}>
                    <Link className='link' to={newTo}>
                    <div className='item' onClick={() => <Item globalState={this.state} item={item}/> }>
                            <div className='image' style= 
                                {{backgroundImage: `url('${item.img}')`}}>
                                <p className='price'>{item.category ==='Sale' || item.category === 'Housing' && item.listing !== 'Roommates' ? '$' : ''}{item.price}</p>
                        ...
                        <span className="editdelete">
                            <a href="#popupEdit"><i className="far fa-edit" onClick={() => {
                                                                                     $('#imagefile').val('')
                                                                                     this.editPost(i)}}
                            ></i></a>
                            <i className="fas fa-trash-alt" onClick={() => this.deletePost(i)}></i>
                        </span>
                </section>
        )
        })
            ...
            <section>
               <div className="manageUser">
                  <div className='userImg'>
                   <img src={this.state.currentUserPhoto} alt='User Photo'></img>
                   </div>
         ...
         <a href="#EditCancel" className="popup__close">&times;</a>
          <h3>Edit</h3>
           <form autoComplete="off" >
			...
			<label className='stylefile'> Change Image
				<input type='file'  id='imagefile' className='editimage' accept="image/*" ></input>
			   </label>
			   <progress id='uploader' value={this.state.progress} max="100">0%</progress>
				</div>
				 </div>
				</form>
                 
				        <a href="#managerUser"><input type='submit' className='submit update' value='Update'></input></a>
        </div>
    </div>
            </section>
